...
void Settings::fadeOutMusic() {
	int volume = i10_cd_colume / 256;
...
void Settings::fadeInMusic() {
	int volume = 0;
	do {
		FSOUND_SetVolumeAbsolute(12, volume);
		Sleep(3);
		volume++;
	} while (volume <= (i10_cd_colume / 256));
}
...
void Settings::unk1() {
	FSOUND_SetVolumeAbsolute(12, i10_cd_colume / 256);

	for (int channel = 0; channel < 12; ++channel)
		FSOUND_SetVolumeAbsolute(channel, i2_soundVolume / 256);
}
